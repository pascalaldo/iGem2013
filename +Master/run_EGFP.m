...
% Integrate ODEs:
tspan = [0 1200]; %(min)
M.values(M.parameters.oxygen) = 9;
d(sprintf('Simulated time: %d minutes', tspan(2)));
...
%for i = 2:length(t)
%    prdt(i) = -prdt(i-1)*(t(i)-t(i-1))*k + (t(i)-t(i-1)) * RATE * rrate(i);
%end
%figure;plot(t,prdt);
...
figure;plot(t,fitfunc(param,t));
hold on;
plot(mxdata,mydata,'r.');
hold off;
